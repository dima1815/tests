...
<givenStories keyword="${keywords.givenStories}">
    <#list givenStories.getStories() as givenStory>
        <givenStory<#if givenStory.hasAnchor()>
                parameters="${givenStory.parameters}"</#if>>${givenStory.path}</givenStory>
    </#list>
</givenStories>
</#macro>
...
<outcomes>
    ...
    <#list outcomes as outcome>
        ...
        <outcome>
            <value>${outcome.description?xml}</value>
            <value><@renderOutcomeValue outcome.getValue() table.getDateFormat()/></value>
            <value>${outcome.matcher?xml}</value>
            <value><#if isVerified == "true">${keywords.yes}<#else>${keywords.no}</#if></value>
        </outcome>
    </#list>
</outcomes>
</#macro>
...
${formattedStep}<#if step.getTable()??>
    <parameter><@renderTable step.getTable()/></parameter></#if>
    <#if step.getFailure()??>
        <failure>${step.failureCause?xml}</failure></#if><#if step.getOutcomes()??><@renderOutcomes step.getOutcomes()/></#if>
</step>
</#macro>
...
<#if story.isCancelled()?string == 'true'>
    <cancelled keyword="${keywords.storyCancelled}" durationKeyword="${keywords.duration}"
               durationInSecs="${story.storyDuration.durationInSecs}"/>
</#if>
